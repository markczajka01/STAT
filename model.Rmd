...
## 1.1 GLM on LDLNTR
```{r}
model_glm <- glm(LDLNTR ~  WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, family = "binomial", data = model_data)
summary(model_glm)
```

## 1.2 Ordinal on LDLRESB
```{r}
ordinal_1 <- polr(LDLRESB ~ WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, data = model_data)
summary(ordinal_1)
...
```{r}
nom_nom_1 <- multinom(LDLRESB ~ WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, data = model_data, trace=FALSE)
summary(nom_nom_1)
...
```{r}
model_glm <- glm(HDLCHREB_BINOM ~  WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, family = "binomial", data = model_data)
summary(model_glm)
```
...
```{r}
ordinal_1 <- polr(HDLCHREB ~ WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, data = model_data)
summary(ordinal_1)
ctable <- coef(summary(ordinal_1))
p <- pnorm(abs(ctable[, "t value"]), lower.tail = FALSE) * 2
ctable <- cbind(ctable, "p value" = p)
...
```{r}
nom_nom_1 <- multinom(HDLCHREB ~ WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, data = model_data, trace=FALSE)
summary(nom_nom_1)
coef(summary(nom_nom_1))
# calculate p-values
zvalues <- summary(nom_nom_1)$coefficients / summary(nom_nom_1)$standard.errors
...
## 3.1 GLM on CHOLNTR
```{r}
model_glm <- glm(CHOLNTR ~  WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, family = "binomial", data = model_data)
summary(model_glm)
```
...
```{r}
ordinal_1 <- polr(CHOLRESB ~ WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, data = model_data)
summary(ordinal_1)
ctable <- coef(summary(ordinal_1))
p <- pnorm(abs(ctable[, "t value"]), lower.tail = FALSE) * 2
ctable <- cbind(ctable, "p value" = p)
...
```{r}
nom_nom_1 <- multinom(CHOLRESB ~ WHOLGR1N + REFGRA1N + FIBRPER + BMISC + AGEC + SLPTIME + SYSTOL + EXLWTBC, data = model_data, trace=FALSE)
summary(nom_nom_1)
coef(summary(nom_nom_1))
# calculate p-values
zvalues <- summary(nom_nom_1)$coefficients / summary(nom_nom_1)$standard.errors
