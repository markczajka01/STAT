...
forest_gump <- randomForest(grouped_LDL_band ~ WG + RG + BMI + age + sleep_time + systolic_blood_pressure, data = train_data, ntree = 500, importance = TRUE)
...
## multinomial
```{r}
nom_nom_2 <- multinom(LDL_status ~ WG + RG + BMI + age + sleep_time + systolic_blood_pressure + exercise_duration + HDL_band + total_cholesterol_band, data = model_data, trace=FALSE)
summary(nom_nom_2)
...
## RF
```{r}
forest_gump_sequel <- randomForest(LDL_status ~ WG + WG_PROP + RG + BMI + age + sleep_time + systolic_blood_pressure, data = train_data, ntree = 500, importance = TRUE)
...
ordinal_3 <- polr(LDL_band ~ WG + RG + BMI + age + sleep_time + systolic_blood_pressure + exercise_duration + HDL_band, data = model_data)
summary(ordinal_3)
...
```{r}
nom_nom_3 <- multinom(LDL_band ~ WG  + RG + BMI + age + sleep_time + systolic_blood_pressure + exercise_duration + HDL_band, data = model_data, trace=FALSE)
summary(nom_nom_3)
...
forest_gump_trilogy <- randomForest(LDL_band ~ WG + RG + BMI + age + sleep_time + systolic_blood_pressure + exercise_duration + HDL_band, data = train_data_1, ntree = 500, importance = TRUE)
...
```{r}
ordinal_4 <- polr(total_cholesterol_band ~ WG + RG + BMI + age + sleep_time + systolic_blood_pressure + exercise_duration + HDL_band, data = model_data)
summary(ordinal_4)
